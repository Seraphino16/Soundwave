export class UserSuccess {
    ...
    static createSuccess(status: string, code: number, message: string, data: any = null) {
        return new UserSuccess(status, code, message, data);
    }

    static userCreated(data: any) {
        return UserSuccess.createSuccess('success', 201, 'Utilisateur créé avec succès', data);
    }
