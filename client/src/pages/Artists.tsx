...
const Artists: React.FC = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const filteredArtists = artistNameFilter
        ? artists.filter((artist: Artist) => artist.name.toLowerCase().includes(artistNameFilter.toLowerCase()))
        : artists;
  ...
  return (
    <>
      <Meta title="Soundwave - Artistes" description="Page de liste des artistes sur SoundWave" />
          ...
          <div className="flex flex-col lg:flex-row gap-6">
            <div className="lg:w-64 lg:border-r lg:pr-6 w-full">
              <div className="flex items-center justify-between lg:hidden mb-4">
                ...
                <button onClick={() => setShowFilters((prev) => !prev)} className="text-sm text-primaryBlue underline" aria-expanded={showFilters}>
              ...
              <div className={`${showFilters ? "block" : "hidden"} lg:block space-y-4`}>
                <div>
                  <label className="block mb-1 text-sm font-medium text-gray-700">Nom artiste</label>
                  <input
                    type="text"
                ...
                <button onClick={handleSearch} className="bg-primaryBlue text-white px-4 py-2 rounded-md hover:bg-blue-600 transition w-full">
                  Rechercher
                </button>
