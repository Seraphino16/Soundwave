...
const AlbumDetail: React.FC = () => {
  ...
  const handleTrackVote = (trackId: string, type: "like" | "dislike") => {
    ...
    toast(type === "like" ? "👍 Vous aimez cette piste" : "👎 Vous n'aimez pas cette piste", {
      position: "bottom-right",
      autoClose: 2000,
    });
  };
  ...
  return (
    <>
      <Meta title={`Soundwave - ${album.title}`} description={`Détails de l'album ${album.title} sur SoundWave`} />
      <div className="flex flex-col items-center justify-center min-h-screen px-4 sm:px-6 lg:px-8">
        <div className="bg-white shadow-xl rounded-lg p-6 sm:p-10 w-full max-w-6xl relative">
          ...
          <div className="mb-10 w-full max-w-4xl mx-auto">
            <h1 className="mt-10 sm:mt-0 text-3xl sm:text-4xl font-bold text-primaryBlue text-center mb-8 sm:mb-10">{album.title}</h1>
                ...
                {album.artists && album.artists.length > 0 && (
                  <p>
                    ...
                    {album.artists.map((artist, index, arr) => (
                      <span key={artist.id}>
                        <Link to={`/artists/${artist.id}`} className="text-primaryBlue hover:underline">
                          {artist.name}
                        </Link>
                        {index < arr.length - 1 && ", "}
                      </span>
          ...
          {album.tracks && album.tracks.length > 0 ? (
                  ...
                  {album.tracks.map((track) => (
                    <tr key={track.id} className="border-b border-gray-100 hover:bg-gray-50">
                      ...
                      <td className="py-2 px-3 text-right flex justify-end items-center gap-3">
                        ...
                        >
                          {favoriteTracks.includes(track.id) ? <FaHeart className="text-red-500 text-lg" /> : <FaRegHeart className="text-lg" />}
                        </button>
                        <button
                          onClick={() => handleTrackVote(track.id, "like")}
                          aria-label="J'aime"
                          className={`p-2 rounded-full transition ${
                            trackVotes[track.id] === "like" ? "bg-green-100 text-green-600" : "hover:text-green-600"
                          }`}
                        >
                          {trackVotes[track.id] === "like" ? <BiSolidLike className="text-xl" /> : <BiLike className="text-xl" />}
                        </button>
                        <button
                          onClick={() => handleTrackVote(track.id, "dislike")}
                          aria-label="Je n'aime pas"
                          className={`p-2 rounded-full transition ${
                            trackVotes[track.id] === "dislike" ? "bg-red-100 text-red-600" : "hover:text-red-600"
                          }`}
                        >
                          {trackVotes[track.id] === "dislike" ? <BiSolidDislike className="text-xl" /> : <BiDislike className="text-xl" />}
                        </button>
                      </td>
                    </tr>
                  ))}
            ...
            <p className="text-center text-gray-500 mb-10">Aucune piste disponible.</p>
          )}
